...
class Generator(nn.Module):
    def __init__(self, cfg: DictConfig):
        ...
        self.analysis_module = UNetResComplex_100Mb(channels = cfg.channels_in)

    def forward(self, mel: torch.Tensor):
        '''
        send logmel spectrogram into analysis module
        '''
        mel = torch.clip(mel, min=1e-8)
        mel = torch.log10(mel)
        out = self.analysis_module(mel)
        if type(out) is dict:
            out = out['mel']
        mel = out + mel
        return {'mel': mel}
...
class VoiceFixer(pl.LightningModule):
    def __init__(self, cfg: DictConfig, channels: int, type_target):
        super(VoiceFixer, self).__init__()

        # self.lr = cfg.learning_rate
        # self.gamma = cfg.lr_decay
        # self.batch_size = cfg.batch_size
        # self.input_segment_length = cfg.input_segment_length
        # self.sample_rate = cfg.sample_rate
        # self.check_val_every_n_epoch = cfg.check_val_every_n_epoch
        # self.

        self.lr = cfg["train"]["learning_rate"]
        self.gamma = cfg["train"]["lr_decay"]
        self.batch_size = cfg["train"]["batch_size"]
        self.input_segment_length = cfg["train"]["input_segment_length"]
        self.sampling_rate = cfg["data"]["sampling_rate"]
        self.check_val_every_n_epoch = cfg["train"]["check_val_every_n_epoch"]
        self.warmup_steps = cfg["train"]["warmup_steps"]
        self.reduce_lr_every_n_steps = cfg["train"]["reduce_lr_every_n_steps"]
